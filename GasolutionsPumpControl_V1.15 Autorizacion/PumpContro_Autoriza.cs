    ...
    public class PumpControl : iProtocolo
    {
        ...
        public PumpControl(string Puerto, Dictionary<byte, RedSurtidor> EstructuraCaras, bool Eco)
        {
            try
            {
                ...
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2016.01.28-2050");//Autorizacion con predeterminacion 99 99 99 99 DCF 28/02/2016
               // SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2016.08.17-1844");//DCF 17-08-2016 Convert.ToDecimal(3.00)
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2016.08.22-1645");////JD TH 22/08/2016
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2016.12.06-1423"); // DCF 12-11-2016 PARA Loop  GST TCP-IP
                SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2018.03.08-1734");//DCF Archivos .txt 08/03/2018  
                SWRegistro.Flush();
                ...
                switch (PropiedadesCara[CaraEncuestada].Estado)
                {
                    ...
                    case (EstadoCara.PumpControlPorAutorizar2):
                        ...
                        if (PropiedadesCara[CaraEncuestada].AutorizarCara == true)
                        {
                            ...
                            string Datos = CaraEncuestada.ToString() + "|" + strLecturasVolumen;
                            //LecturaInicialVenta(CaraEncuestada, strLecturasVolumen);
                            ThreadPool.QueueUserWorkItem(new WaitCallback(EnviarLecturaInicial), Datos); //JD TH 22/08/2016                      
                            ...
                            if (PropiedadesCara[CaraEncuestada].ValorPredeterminado > 0)//DCF 17-08-2016 Convert.ToDecimal(3.00)
                            {
                            ...
                            //Autorizacion con predeterminacion 99 99 99 99 DCF 28/02/2016
                            //else if (PropiedadesCara[CaraEncuestada].ValorPredeterminado <= 0 &&
                            //    PropiedadesCara[CaraEncuestada].VersionFirmware >= Convert.ToDecimal(1.12) &&
                            //    PropiedadesCara[CaraEncuestada].VersionFirmware <= Convert.ToDecimal(3.00))
                            //DCF 17-08-2016 Convert.ToDecimal(3.00)
                            ...
                            else
                                 {
                                    ...
                                    do
                                    {
                                        PropiedadesCara[CaraEncuestada].ValorPredeterminado = 999999;

                                        if (!ProcesoEnvioComando(ComandoSurtidor.Predeterminar))
                                        {
                                            SWRegistro.WriteLine(DateTime.Now + "|" + CaraEncuestada + "|Error|No respondió Autorizacion con Predeterminar");
                                            SWRegistro.Flush();
                                        }
                                    ...
                                    if (ComandoAceptado == false)
                                    {
                                        SWRegistro.WriteLine(DateTime.Now + "|" + CaraEncuestada + "|Error|Comando Autorizacion con Predetermina fallido. Valor: $" +
                                           PropiedadesCara[CaraEncuestada].ValorPredeterminado);
                                        SWRegistro.Flush();
                                    }
                                    ...
                                    do
                                    {
                                        ...
                                        if (!ProcesoEnvioComando(ComandoSurtidor.Estado))
                                        {
                                            SWRegistro.WriteLine(DateTime.Now + "|" + CaraEncuestada + "|Error|No respondio a comando Estado despues de Autorizacion con Predeterminar");
                                            SWRegistro.Flush();
                                        }
                                    } while (PropiedadesCara[CaraEncuestada].Estado != EstadoCara.PumpControlIniciaDespacho &&
                                       PropiedadesCara[CaraEncuestada].Estado != EstadoCara.PumpControlDespacho && Reintentos <= 2);
        ...
        private void EnviarLecturaInicial(object Datos)
        {

            try
            {
                string[] valores = Datos.ToString().Split('|');
                byte Cara =Convert.ToByte (valores[0]);
                string Volumen = valores[1];
                LecturaInicialVenta(Cara, Volumen);

            }
            catch (Exception Excepcion)
            {
                
                string MensajeExcepcion = "EnviarLecturaInicial: " + Excepcion;
                SWRegistro.WriteLine(DateTime.Now + "|" + CaraEncuestada + "|Excepcion|" + MensajeExcepcion);
                SWRegistro.Flush();
            }
