...
{
    public class Gilbarco_PumaLAN: iProtocolo
    {
        ...
        public Gilbarco_PumaLAN(string Puerto, Dictionary<byte, RedSurtidor> EstructuraCaras, bool Eco)
        {
            try
            {
                ...
                Archivo = Application.StartupPath + "/LogueoProtocolo/" + DateTime.Now.ToString("yyyyMMdd") + "-Gilbarco_PumaLAN-Sucesos(" + Puerto + ").txt";
                ...
                ArchivoTramas = Application.StartupPath + "/LogueoProtocolo/" + DateTime.Now.ToString("yyyyMMdd") + "-Gilbarco_PumaLAN-Tramas.(" + Puerto + ").txt";
                ...
                SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo Gilbarco_PumaLAN. Modificado 01.11.2015-0236");//Utiliza COnversor de Protocolo Gilbarco - PumalAN  DCF 01/11/2015
        ...
        public Gilbarco_PumaLAN(bool EsTCPIP, string DireccionIP, string Puerto, Dictionary<byte, RedSurtidor> EstructuraCaras, bool Eco)
        {
            try
            {
                if (!Directory.Exists(Application.StartupPath + "/LogueoProtocolo"))
                ...
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo Gilbarco_Extended_TCPIP. Modificado 27.03.2018-1812");// ADAPTADO PARA SAUCE
                SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo Gilbarco_Extended_TCPIP. Modificado 27.03.2018-1812");// ADAPTADO PARA SAUCE
                SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Numero de caras: " + EstructuraRedSurtidor.Count);
                SWRegistro.Flush();
        ...
        public void EnviarComando()
        {
            try
            {
                if (PuertoCom.IsOpen)
                {
                    ...
                    Thread.Sleep(200);//para no escribir TX continuamente luego de leer RX 
...
"|" + CaraID + "|*8|Verificando conexion 2 " + EsInicializado);
        ...
        public void TomarAccion()
        {
            try
            {
                ...
                switch (EstructuraRedSurtidor[CaraEncuestada].Estado)
                {
                    ...
                    case (EstadoCara.FinDespachoB): // EstadoCara.FinDespachoB;
                ...
                if (ProcesoEnvioComando(ComandoSurtidor.TotalDespacho, false))
                {
                    ...
                    if (EstructuraRedSurtidor[CaraEncuestada].ListaGrados.Count - 1 >= EstructuraRedSurtidor[CaraEncuestada].GradoVenta)
                    {
                        ...
                        if (EstructuraRedSurtidor[CaraEncuestada].Volumen != 0)
                        {
                            ...
                            //Guardo la lectura inicial Igual a la lectura finla del despacho, esto se hace porque el conversor el conversor no envia el cambi de estado POR AUTORIZAR  DCF 01/11/2015
                            EstructuraRedSurtidor[CaraEncuestada].ListaGrados[EstructuraRedSurtidor[CaraEncuestada].GradoVenta].LecturaInicialVenta =
                            EstructuraRedSurtidor[CaraEncuestada].ListaGrados[EstructuraRedSurtidor[CaraEncuestada].GradoVenta].LecturaFinalVenta; 
        ...
        public void RecuperarDatosFindeVenta()
        {
            try
            {
                ...
                if (LRCObtenidoEnTama == LRCCalculado)//Eco
                {
                   ...
                   // if (CaraqueResponde == CaraEncuestada) //********** siempre envia la cara 1 E0 ?? es el conversor de protocolo se quita esta validacion  ***** DCF- 01/11/2015 
                    {
                        ...
                        {
                               SWRegistro.WriteLine(DateTime.Now + "|" + CaraID + "|Inconsistencia|Este surtidor no Soporta Gilbarco_Extended");
                        }
                        else
                        {
                    ...
                    //        + " no corresponde a Cara que responde: " + CaraqueResponde + " - CaraEncuestada: " + CaraEncuestada);
                    //    SWRegistro.Flush();
                    //}
                }
                else
        ...
        public void VerifySizeFile()
        {
            try
            {
                ...
                if (FileInf.Length > 30000000)
                {
                    SWRegistro.Close();
                    //Crea archivo para almacenar inconsistencias en el proceso logico
                    Archivo = Application.StartupPath + "/LogueoProtocolo/" + DateTime.Now.ToString("yyyyMMdd") + "-Gilbarco_PumaLAN-Sucesos(" + PuertoCom.PortName + ").txt";
                    SWRegistro = File.AppendText(Archivo);
                }
      ...
      //  public void Evento_VentaAutorizada(byte Cara, string Precio, string ValorProgramado, byte TipoProgramacion, string Placa, int MangueraProgramada, bool EsVentaGerenciada)
       public void Evento_VentaAutorizada(byte Cara, string Precio, string ValorProgramado, byte TipoProgramacion, string Placa, int MangueraProgramada, bool EsVentaGerenciada, string guid, Decimal PresionLLenado)
        ...
        public void SolicitarLecturasSurtidor(ref string Lectura, string Surtidor)
        {
            //
        }
