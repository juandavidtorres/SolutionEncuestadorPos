...
namespace POSstation.Protocolos
{
    public class PumpControl : iProtocolo
    {
        ...
        public enum ComandoPresion
        {
            Set_P_Max, //Set Presión Máxima
            Lec_P_Max, //Lectura de presión Máxima
            Set_P_Min, //Set Presión Mínima
            Lec_P_Min, //Lectura Presión Mínima
            Auto_Presion, //Autorizacion de controlador de presion 
            DesAuto_Presion, //Desautorizar	
            Set_Point_presion, //Set Point De Presión
            Lect_Set_P, //Lectura de Set P de presión 
            Tiempo_espera, //Tiempo de espera 
            Reset_P, //Reset del CP
            Mac, //MAc del controlador eth
            Ver //Version del equipo
        }
        ...
        byte[] TramaTx_CPT = new byte[5];
        byte[] TramaRx_CPT = new byte[5];  
        ...
        TcpClient Cliente_CPT;
        NetworkStream Stream_CPT;


        string dir_CPT = "192.168.1.199";
        int Puert_CPT = 7000;
        ...
        string ArchivoTramas_CPT;
        StreamWriter SWTramas_CPT; // para la escritura de los comando del controlador de presion

        int Error_Anterior = 0;


        string strDatosPredeterminado = "0200.00";

        Decimal PresionLLenado_ = 0.0m;
        bool reenviar_setP = false; 
        ...
        public PumpControl(string Puerto, Dictionary<byte, RedSurtidor> EstructuraCaras, bool Eco)
        {
            try
            {
                ...
                ArchivoTramas_CPT = Application.StartupPath + "/LogueoProtocolo/" + DateTime.Now.ToString("yyyyMMdd") + "-Presion-Tramas_CPT (" + Puert_CPT + ").txt";
                SWTramas_CPT = File.AppendText(ArchivoTramas_CPT);
               ...
               // SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2016.11.04-1538");//paridad Even 
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2017.03.15-1452");////DCF PresionLLenado para peru  09/03/2017
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2018.02.23-1653 *");//DCF 23/02/2018
                SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2018.03.15-1532");//DCF Archivos .txt 08/03/2018  
              
                SWRegistro.Flush();


       
                foreach (RedSurtidor oCara in EstructuraCaras.Values)
                {
                    foreach (Grados oGrado in EstructuraCaras[oCara.Cara].ListaGrados)
                        SWRegistro.WriteLine(DateTime.Now + "|" + oCara.Cara + "|Inicio|Grado: " + oGrado.NoGrado + " - Manguera: " + oGrado.MangueraBD +
                            " - IdProducto: " + oGrado.IdProducto + " - Precio: " + oGrado.PrecioNivel1 + " Grado Venta Parcial: " + oCara.GradoMangueraVentaParcial + " AplicaControlPresionLLenado: " + oCara.AplicaControlPresionLLenado);
                }
                SWRegistro.Flush();
                ...
                //Conexion con el controlador de presion CPT
                try
                {
                    dir_CPT = "192.168.1.199";// por lo pronto se deja directo la ip 
                    Puert_CPT = 7000;

                    Cliente_CPT = new TcpClient(dir_CPT, Puert_CPT);
                    Stream_CPT = Cliente_CPT.GetStream();


                    SWRegistro.WriteLine(DateTime.Now + "| Inicio|Coneccion CPT establecidad- direccion :" + dir_CPT);


                }
                catch (Exception e)
                {
                    string MensajeExcepcion = "No se pudo Crear la conexión con el Server CPT: " + dir_CPT + ":  " + Puert_CPT + e;
                ...
                if (!PuertoCom.IsOpen)
                {
                    ...
                    PuertoCom.Parity = Parity.Even; //paridad Even 
        ...
        //(EsTCPIP, DireccionIP, Puerto, EstructuraRedSurtidor, true, true);
        public PumpControl(bool TCPIP_ETH, string DireccionIP_ETH, string Puerto_ETH, Dictionary<byte, RedSurtidor> EstructuraCaras, bool Eco)
        {
            try
            {
                ...
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2016.11.04-1538");//paridad Even 
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2017.03.15-1452");////DCF PresionLLenado para peru  09/03/2017
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2018.02.23-1120");//DCF 23/02/2018
                //SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2018.0308-1841");//DCF Archivos .txt 08/03/2018  
                SWRegistro.WriteLine(DateTime.Now + "|0|Inicio|Protocolo PumpControl TCP_IP modificado 2018.03.15-1532");//DCF Archivos .txt 08/03/2018  
                SWRegistro.Flush();
        ...
        private void Armar_TramaCP( ComandoPresion ComandoCP)
        {

                //Dependiendo del Comando a Enviar se Construye la Trama
            switch (ComandoCP)
            {
                case ComandoPresion.Set_Point_presion:

                    //Envio el set de presion al CPT para su control de sobre presion. DCF 04/01/2018 Gamarra
                    //WPs 0203.65 ~	57 50 73 30 32 30 33 2E 36 35 7E
                    //                             W     P      s     0     0     0     0     .    0     0     ~
                    TramaTx_CPT = new byte[11] { 0x57, 0x50, 0x73, 0x00, 0x00, 0x00, 0x00, 0x2E, 0x00, 0x00, 0x7E };
                    //                                              0      1     2     3          5     6  

                    byte[] intBytes = Encoding.ASCII.GetBytes(strDatosPredeterminado.Substring(0, 1));
                    TramaTx_CPT[3] = Convert.ToByte(intBytes[0]);

                    intBytes = Encoding.ASCII.GetBytes(strDatosPredeterminado.Substring(1, 1));
                    TramaTx_CPT[4] = Convert.ToByte(intBytes[0]);

                    intBytes = Encoding.ASCII.GetBytes(strDatosPredeterminado.Substring(2, 1));
                    TramaTx_CPT[5] = Convert.ToByte(intBytes[0]);

                    intBytes = Encoding.ASCII.GetBytes(strDatosPredeterminado.Substring(3, 1));
                    TramaTx_CPT[6] = Convert.ToByte(intBytes[0]);

                    intBytes = Encoding.ASCII.GetBytes(strDatosPredeterminado.Substring(5, 1));
                    TramaTx_CPT[8] = Convert.ToByte(intBytes[0]);

                    intBytes = Encoding.ASCII.GetBytes(strDatosPredeterminado.Substring(6, 1));
                    TramaTx_CPT[9] = Convert.ToByte(intBytes[0]); 


                break;


                case ComandoPresion.Lect_Set_P:

                TramaTx_CPT = new byte[4] {0x52, 0x50, 0x73, 0x7E }; //RPs~
        ...
        public void EnviarComando_Presion( ComandoPresion ComandoCP)
        {
            try
            {
                ...
                try
                {
                    Armar_TramaCP( ComandoCP ); 
                    
                    VerificarConexion_CPT();
                    Stream_CPT.Write(TramaTx_CPT, 0, TramaTx_CPT.Length);
                    Stream_CPT.Flush();
                ...
                catch (System.Net.Sockets.SocketException)//Si genera error lo capturo, espero y reenvio el comando
                {
                    ...
                    catch (Exception)
                    {
                        ...
                        SWRegistro.WriteLine(DateTime.Now + "|0|EnviarComando_Presion - No respondio al comando:  " + ComandoCP.ToString());
                        SWTramas.Flush();
                ...
                //LOGUEO DE TRAMA TRANSMITIDA
                string strTrama = "";
                for (int i = 0; i <= TramaTx_CPT.Length - 1; i++)
                    strTrama += TramaTx_CPT[i].ToString("X2") + "|";


                SWTramas_CPT.WriteLine(
                    DateTime.Now.Day.ToString().PadLeft(2, '0') + "/" + DateTime.Now.Month.ToString().PadLeft(2, '0') + "/" +
                    DateTime.Now.Year.ToString().PadLeft(4, '0') + "|" +
                    DateTime.Now.Hour.ToString().PadLeft(2, '0') + ":" + DateTime.Now.Minute.ToString().PadLeft(2, '0') + ":" +
                    DateTime.Now.Second.ToString().PadLeft(2, '0') + "." + DateTime.Now.Millisecond.ToString().PadLeft(3, '0') +
                    "|" + CaraEncuestada + "|Tx| " + ComandoCP + ": "+ strTrama);
        ...
        public void RecibirInformacion_Presion()
        {
            try
            {
                

                if (!Stream_CPT.DataAvailable)
                    Thread.Sleep(40);

                if (Stream_CPT.DataAvailable)
                {

                    byte[] TramaRxTemporalCPT = new byte[500];

                    if (Stream_CPT.CanRead)
                    {
                        do
                        {
                            //Cambio en en el tiempo de espera de la lectura del buffer TCP //2013-03-27 0812
                            Bytes_leidosCPT = Stream_CPT.Read(TramaRxTemporalCPT, 0, TramaRxTemporalCPT.Length);

                        } while (Stream_CPT.DataAvailable);
                    }
                    ...
                    byte[] TramaTemporalCPT = new byte[Bytes_leidosCPT];
                    ...
                    TramaRx_CPT = new byte[TramaTemporalCPT.Length];

                    //Almacena los datos reales (sin eco) en TramaRx
                    for (int i = 0; i <= (TramaTemporalCPT.Length - 1); i++)
                        TramaRx_CPT[i] = TramaRxTemporalCPT[i];


                    ///////////////////////////////////////////////////////////////////////////////
                    //LOGUEO DE TRAMA RECIBIDA
                    string strTramaCPT = "";
                    for (int i = 0; i <= TramaRx_CPT.Length - 1; i++)
                    {
                     //strTramaCPT += TramaRx_CPT[i].ToString("X2") + "|"; 
                        strTramaCPT += Char.ConvertFromUtf32(TramaRx_CPT[i]); // convertir a ASCII
                    }
                  ...
                  //  LimpiarSockets_CPT();//Borro de memoria el cliente TCP-IP ''Juan David Torres
                    FalloComunicacion = false;
            ...
            {
                string MensajeExcepcion = "Excepcion en el Metodo RecibirInformacion_Presion: " + Excepcion;
                SWRegistro.WriteLine(DateTime.Now + "|" + CaraEncuestada + "|Excepcion|" + MensajeExcepcion);
                SWRegistro.Flush();
            }
        }
        ...
        public void VerifySizeFile()
        {
            try
            {
                ...
                FileInf = new FileInfo(ArchivoTramas_CPT);
                if (FileInf.Length > 50000000)
                {
                    SWTramas_CPT.Close();
                    ArchivoTramas_CPT = Application.StartupPath + "/LogueoProtocolo/" + DateTime.Now.ToString("yyyyMMdd") + "-Presion-Tramas_CPT (" + Puert_CPT + ").txt";
        ...
        public void VerificarConexion_CPT()
        {
            int iReintento = 0;
            string Comando = "";
            try
            {
                if (Cliente_CPT == null)
                {
                    Boolean EsInicializado_CPT = false;
                    while (!EsInicializado_CPT)
                    {
                        try
                        {
                            ...
                            if (Cliente_CPT == null)
                            {
                                SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "|No inicializo - Ip: " + dir_CPT + " Puert_CPT: " + Puert_CPT);
                                SWTramas_CPT.Flush();
                                Thread.Sleep(200);
                            }
                            else
                            {
                                SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "| Ip CPT: " + dir_CPT + " Puert_CPT: " + Puert_CPT);
                                SWTramas_CPT.Flush();
                                Thread.Sleep(200);
                            }
                        }
                        ...
                        {
                            SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "|Falla de inicializacion - Ip: " + dir_CPT + " Puert_CPT: " + Puert_CPT + " Mensaje: " + e.Message);
                            SWTramas_CPT.Flush();
                            Thread.Sleep(200);
                        }
                        ...
                        if (Cliente_CPT != null)
                        {
                            //SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "|Inicializada - Ip: " + dir_CPT + " Puert_CPT: " + Puert_CPT);
                            //SWTramas_CPT.Flush();
                            EsInicializado_CPT = true;
                        }
                    }
                }

                Boolean estadoAnterior_CPT = true;
                if (!this.Cliente_CPT.Client.Connected)
                {
                    estadoAnterior_CPT = false;
                    SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion CPT|" + Comando + "|Perdida de comunicacion - BeginDisconnect");
                    SWTramas_CPT.Flush();

                    try
                    {
                        Cliente_CPT.Client.BeginDisconnect(true, callBack, Cliente_CPT);
                    ...
                    catch (Exception e)
                    {
                        ...
                        SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion CPT|" + Comando + "|Falla BeginDisconnect: " + e.Message);
                        SWTramas_CPT.Flush();
                        Thread.Sleep(200);
                    }
                }
                ...
                {
                    //SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "|Conexion Abierta");
                    //SWTramas_CPT.Flush();
                    estadoAnterior_CPT = true;
                }



                while (!this.Cliente_CPT.Client.Connected)
                {
                    try
                    {
                        iReintento = iReintento + 1;
                        SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "|Perdida de comunicacion - Intento Reconexion CPT: " + iReintento.ToString());
                        SWTramas_CPT.Flush();


                        Cliente_CPT.Client.BeginConnect(Dns.GetHostAddresses(this.dir_CPT), Convert.ToInt16(this.Puert_CPT), callBack, Cliente_CPT);
                        //Cliente_CPT.Client.Connect(Dns.GetHostAddresses(this.dir_CPT), Convert.ToInt16(this.Puert_CPT));
                    ...
                    {//Reintento de conexcion para el caso de Cruz roja
                        ...
                        //SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "|Falla BeginConnect-Creando Socket: " + ex.Message);
                        //SWTramas_CPT.Flush();
                        SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion CPT |" + Comando + "|BeginConnect-Creando Socket: Abriendo nuevamente la conexcion");
                        SWTramas_CPT.Flush();

                        AbrirSocketReintento_CPT();

                    }
                    catch (Exception)
                    {
                        //SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "|Falla BeginConnect: " + e.Message);
                        //SWTramas_CPT.Flush();

                        SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + Comando + "|BeginConnect: Abriendo nuevamente la conexcion");
                        SWTramas_CPT.Flush();
                ...
                this.Stream = Cliente_CPT.GetStream();

                if (!estadoAnterior_CPT)
                {
                    SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion CPT |" + Comando + "|Reconexion establecida");
                    SWTramas_CPT.Flush();
                }
            }
            catch (Exception exec)
            {
                SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion CPT |" + Comando + "|Excepcion|" + exec.Message);
                SWTramas_CPT.Flush();
            }
        }
        ...
        public void LimpiarSockets()
        {
            try
            {
                ...
                SWRegistro.WriteLine(DateTime.Now + "|LimpiarSockets ****************************:" );
                SWRegistro.Flush();
            }
            catch (Exception ex)
            {
        ...
        public void LimpiarSockets_CPT()
        {
            try
            {
                //ClientePumpControl.Client.Disconnect(false);  
                Cliente_CPT.Client.Close();
                Cliente_CPT.Close();
                Stream_CPT.Close();
                Stream_CPT.Dispose();
                Stream_CPT = null;
            ...
            {
                SWTramas_CPT.WriteLine(DateTime.Now + "|LimpiarSockets CPT :" + ex.Message);
                SWTramas_CPT.Flush();
        ...
        void LimpiarVariableSocket_CPT()
        {
            try
            {
                Cliente_CPT.Close();
            ...
            {
                SWTramas_CPT.WriteLine(DateTime.Now + "|Metodo|" + "LimpiarVariableSocket CPT : " + "Mensaje: " + ex.Message);
                SWTramas_CPT.Flush();
            }
        ...
        void AbrirSocketReintento_CPT()
        {
            try
            {
                Thread.Sleep(20);
                LimpiarVariableSocket_CPT();//Libero los recursos antes de iniciar una nueva conexcion con la veeder
                Cliente_CPT = new TcpClient(dir_CPT, Convert.ToInt16(Puert_CPT));
                Stream_CPT = Cliente_CPT.GetStream();
                if (this.Cliente_CPT.Client.Connected == true)
                {
                    SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + "|Conexion Abierta CPT ");
                    SWTramas_CPT.Flush();
                }
                else
                {
                    SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + "|Conexion Cerrada CPT ");
                    SWTramas_CPT.Flush();
                }
            ...
            {
                SWTramas_CPT.WriteLine(DateTime.Now + "|Conexion|" + "|Falla AbrirSocketReintento  Creando Socket : " + ex.Message);
                SWTramas_CPT.Flush();
        ...
        private void AnalizarTrama_CPT(ComandoPresion ComandoCP)
        {
            try
            {            
            ...
            {
                string MensajeExcepcion = "Excepcion en el Metodo AnalizarTrama_CPT: " + Excepcion;
                SWRegistro.WriteLine(DateTime.Now + "|" + CaraEncuestada + "|Excepcion|" + MensajeExcepcion);
                SWRegistro.Flush();
            }
        }
            ...
            try
            {
                ...
                switch (PropiedadesCara[CaraEncuestada].Estado)
                {
                    ...
                    case (EstadoCara.PumpControlDespacho):
                        ...
                        //////DCF PresionLLenado para peru  09/03/2017
                        //if(PropiedadesCara[CaraEncuestada].SetPresionLLenado != 0 && PropiedadesCara[CaraEncuestada].AplicaControlPresionLLenado)
                        //{
                            
                        //    PropiedadesCara[CaraEncuestada].ListaGrados[0].PresionLlenado = 0; //reset el valor 

                        //    ProcesoEnvioComando(ComandoSurtidor.ObtenerPresion);//OBTENER LA PRESION DE LLENADO 

                        //        if( PropiedadesCara[CaraEncuestada].ListaGrados[0].PresionLlenado >=PropiedadesCara[CaraEncuestada].SetPresionLLenado)
                        //        {
                        //            ProcesoEnvioComando(ComandoSurtidor.DetenerVenta);
                        //            SWRegistro.WriteLine(DateTime.Now + "|" + CaraEncuestada + "|Proceso|Comando de Detencion enviado Por Sobre Presion alcanzada");
                        //            SWRegistro.Flush();
                        //        }
                        //}
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            string strPrecio = PropiedadesCara[CaraEncuestada].PrecioCara.ToString("N");
        ...
        public void Evento_VentaAutorizada(byte Cara, string Precio, string ValorProgramado, byte TipoProgramacion, string Placa, int MangueraProgramada, bool EsVentaGerenciada, string guid, Decimal PresionLLenado) //DCF PresionLLenado para peru  09/03/2017
       {
            try
            {
                ...
                if (PropiedadesCara.ContainsKey(Cara))
                {
                    ...
                    } while (reenviar_setP && reintentos < 3);


                    if (reintentos >= 3)
                   {
                       SWRegistro.WriteLine(DateTime.Now + "|" + Cara + "|Evento| Cancelación de Autorización no se estableció Control de presión");//DCF PresionLLenado para peru  09/03/2017
                          SWRegistro.Flush();
                    ...
                    PropiedadesCara[Cara].SetPresionLLenado = PresionLLenado; //DCF PresionLLenado para peru  09/03/2017

                    PresionLLenado_ = PresionLLenado; 

                    strDatosPredeterminado = Convert.ToString(PresionLLenado).PadLeft(7, '0');

                    
                    SWRegistro.WriteLine(DateTime.Now + "|" + Cara + "|Evento|Recibe Autorizacion. Valor Programado " + ValorProgramado +
                        " - Tipo de Programacion: " + TipoProgramacion + " - PresionLLenado: " + PropiedadesCara[Cara].SetPresionLLenado.ToString() + " -Longitud de Presion: " + strDatosPredeterminado.Length);//DCF PresionLLenado para peru  09/03/2017
                    SWRegistro.Flush();
        ...
        //}
          ...
          public void  Envio_SetPresion()
         {

            //DCF 23/02/2018
            EnviarComando_Presion(ComandoPresion.Set_Point_presion ); // envia el valor del set al controlador de presion    
            RecibirInformacion_Presion();        
            EnviarComando_Presion(ComandoPresion.Lect_Set_P);//Consulta la presion seteada para el control 
            Thread.Sleep(3200);
            RecibirInformacion_Presion();
            ...
            for (int j = 0; TramaRx_CPT[j] != 0x0D; j++)
            {
                if (TramaRx_CPT[j] ==     'S')
                if (TramaRx_CPT[j + 1] == 'e')
                if (TramaRx_CPT[j + 2] == 't')
                if (TramaRx_CPT[j + 3] == '_')
                if (TramaRx_CPT[j + 4] == 'B')
                if (TramaRx_CPT[j + 5] == 'a')
                if (TramaRx_CPT[j + 6] == 'r')
                if (TramaRx_CPT[j + 7] == ' ')
                if (TramaRx_CPT[j + 8] == '=')
                    {
                        for (int k = j + 9; TramaRx_CPT[k] != 0x20; k++)
                        {
                            if (punto == true)
                                div = div * 10;
                            set += Char.ConvertFromUtf32(TramaRx_CPT[k]); // convertir a ASCII

                            if (TramaRx_CPT[k] == 0x2e)
                            {
                                punto = true;
                            }
                        }

                        val2 = Convert.ToDecimal(set);
                        val2 = val2 / div;


                        if (val2 == PresionLLenado_)
                        {
                            reenviar_setP = false; // si la presin set es igual a la que llega del sistema desaciva el renvio
                        }
                        else
                            reenviar_setP = true; 
