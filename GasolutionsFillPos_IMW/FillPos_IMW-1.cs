    ...
    public class FillPos_IMW: iProtocolo
    {
        ...
        public FillPos_IMW(bool EsTCPIP, string DireccionIP, string Puerto, Dictionary<byte, RedSurtidor> EstructuraCaras, bool Eco)
        {
            ...
            {                                
                ...
                Archivo = Application.StartupPath + "/LogueoProtocolo/" + DateTime.Now.ToString("yyyyMMdd") + "-FillPos_IMW-Sucesos(" + Puerto + ").txt";
                ...
                ArchivoTramas = Application.StartupPath + "/LogueoProtocolo/" + DateTime.Now.ToString("yyyyMMdd") + "-FillPos_IMW-Tramas.(" + Puerto + ").txt";
            ...
            ArmarTramaTx(ComandoSurtidor ComandoTx, bool PrecioNivel1)
        {
            try
            {
                ...
                switch (ComandoTx)
                    {
                        case (ComandoSurtidor.Heartbeat):  // se envia la autorizacion  bit 2 y bit 1 es el HeartBeat
                              Register = 3155;   //Direcion del registro para los pulso de vida
                              Value_Registo =3; // 3 = 00000011// 1 envia el bit 1 en 00000001
                              Tipo = true; //Escritura
                             break;
        ...
        public void EnviarComando_TCPIP()
        {
            try
            {
                ...
                //using (ClienteFillPos_IMW = new TcpClient(DireccionIP, Puerto))
                
                 {
                    ...
                    if (Tipo)
                    {
                      ...
                      master.WriteSingleRegister(CaraID, Register, Value_Registo);                        
                    }
                    else
                    {
                ...
                SWTramas.WriteLine(
                    ...
                    DateTime.Now.Hour.ToString().PadLeft(2, '0') + ":" + DateTime.Now.Minute.ToString().PadLeft(2, '0') + ":" +
                    DateTime.Now.Second.ToString().PadLeft(2, '0') + "." + DateTime.Now.Millisecond.ToString().PadLeft(3, '0') +
                    "|" + CaraID + "|Tx|" + "Register = " + (Register + 1) + " -Value_Registo = " + Value_Registo  + " -Numeros_Registro = " + Numeros_Registro );
        ...
        public void RecibirInformacion_TCPIP()
        {
            try
            {
                ...
                switch (ComandoCaras)
                {
                    ...
                    case (ComandoSurtidor.Heartbeat):
                        Register = Convert.ToUInt16(3149 - 1);//se resta u Register = 3149;
                    break;
            ...
            try
            {
                switch (ComandoCaras)
                {
                    ...
                    case (ComandoSurtidor.Heartbeat):
                        ...
                        out_pos_hb = Convert.ToInt32(values[0]) ^ 0x80;
                         out_pos_hb = Convert.ToInt32(values[0]) & 0x80;
                         out_pos_hb = Convert.ToInt32(values[0]) | 0x80;
        ...
        public void VerifySizeFile()
        {
            try
            {
                ...
                if (FileInf.Length > 30000000)
                {
                    SWRegistro.Close();
                    //Crea archivo para almacenar inconsistencias en el proceso logico
                    Archivo = Application.StartupPath + "/LogueoProtocolo/" + DateTime.Now.ToString("yyyyMMdd") + "-Gilbarco_Extended-Sucesos(" + PuertoCom.PortName + ").txt";
                    SWRegistro = File.AppendText(Archivo);
                }
